...
function addCart(){
	...
	if (isset($_GET['add_cart'])) {
		...
		$run_check=mysqli_connect($db,$check_product);
...
function getPro(){
	...
	while ($row_product=mysqli_fetch_array($run_products)) {
		...
		echo "<div class='col-md-4 col-sm-6'>
			...
			<div class='text'>
			<h3><a herf='details.php?pro_id=$pro_id'>$pro_title
			</a>
			</h3>
...
function getPcatPro(){
	...
	if(isset($_GET['p_cat'])){
		...
		$get_products="select * from products where p_cart_id=$p_cat_id";
		$run_products=mysqli_query($db,$get_products);
		$count=mysqli_num_rows($run_products);
		if ($count==0) {
			echo "
			<div class='box'>
			<h1>No Product is found In this product category</h1></div>
			";
		}else{
			echo "<div class='box'>
			<h1>$p_cat_title</h1>
...
function getcatPro(){
	...
	if(isset($_GET['cat'])){
		...
		$get_products="select * from products where cart_id=$cat_id";
		$run_products=mysqli_query($db,$get_products);
		$count=mysqli_num_rows($run_products);
		if ($count==0) {
			echo "
